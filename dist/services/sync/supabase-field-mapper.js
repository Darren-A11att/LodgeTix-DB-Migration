...
exports.SUPABASE_TO_MONGO_MAPPINGS = void 0;
exports.mapSupabaseToMongo = mapSupabaseToMongo;
exports.mapMongoToSupabase = mapMongoToSupabase;
 ...
 */
exports.SUPABASE_TO_MONGO_MAPPINGS = {
 ...
 */
function mapSupabaseToMongo(supabaseDoc, collectionName, options = {}) {
    const mappings = exports.SUPABASE_TO_MONGO_MAPPINGS[collectionName];
    if (!mappings) {
    ...
    for (const mapping of mappings) {
        ...
        switch (mapping.type) {
            ...
            case 'objectid':
                ...
                if (mongodb_1.ObjectId.isValid(sourceValue)) {
                    transformedValue = new mongodb_1.ObjectId(sourceValue);
                }
                else {
 ...
 */
function mapMongoToSupabase(mongoDoc, collectionName) {
    const mappings = exports.SUPABASE_TO_MONGO_MAPPINGS[collectionName];
    if (!mappings) {
        console.warn(`No mapping configuration found for collection: ${collectionName}`);
        return mongoDoc;
    ...
    for (const mapping of mappings) {
        ...
        let transformedValue = sourceValue;
        if (sourceValue instanceof mongodb_1.ObjectId) {
 ...
 */
function getFieldMapping(collectionName, fieldName, direction = 'supabase-to-mongo') {
    const mappings = exports.SUPABASE_TO_MONGO_MAPPINGS[collectionName];
    if (!mappings)
        return undefined;
 ...
 */
function validateRequiredFields(doc, collectionName, source = 'supabase') {
    const mappings = exports.SUPABASE_TO_MONGO_MAPPINGS[collectionName];
    if (!mappings) {
 ...
 */
function generateMappingReport(collectionName) {
    const collections = collectionName
        ? [collectionName]
        : Object.keys(exports.SUPABASE_TO_MONGO_MAPPINGS);
    let report = '# Supabase to MongoDB Field Mapping Report\n\n';
    report += 'Generated: ' + new Date().toISOString() + '\n\n';
    for (const collection of collections) {
        const mappings = exports.SUPABASE_TO_MONGO_MAPPINGS[collection];
        if (!mappings)
            continue;
