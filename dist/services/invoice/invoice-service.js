...
const invoice_generator_factory_1 = require("./invoice-generator-factory");
class InvoiceService {
    /**
     * Generate a customer invoice based on registration type
     */
    static async generateCustomerInvoice(options) {
        const { registration } = options;
        // Create appropriate generator based on registration type
        const generator = invoice_generator_factory_1.InvoiceGeneratorFactory.createFromRegistration(registration);
     ...
     */
    static async generateSupplierInvoice(customerInvoice, options) {
        // Create supplier invoice generator
        const generator = invoice_generator_factory_1.InvoiceGeneratorFactory.createSupplierGenerator();
     ...
     */
    static async generateInvoicePair(options) {
     ...
     */
    static async generateInvoice(type, options, customerInvoice) {
        if (type === 'customer') {
     ...
     */
    static validateInvoiceData(payment, registration) {
        const errors = [];
        // Validate payment
        if (!payment) {
     ...
     */
    static getRegistrationType(registration) {
        return invoice_generator_factory_1.InvoiceGeneratorFactory.getRegistrationTypeDisplay(registration);
    }
    /**
     * Check if a registration is for individuals
     */
    static isIndividualsRegistration(registration) {
        return invoice_generator_factory_1.InvoiceGeneratorFactory.isIndividualsRegistration(registration);
    }
    /**
     * Check if a registration is for a lodge
     */
    static isLodgeRegistration(registration) {
        return invoice_generator_factory_1.InvoiceGeneratorFactory.isLodgeRegistration(registration);
    }
}
