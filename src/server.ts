...
app.get('/api/invoices/pending', async (req, res) => {
  try {
    ...
    // Get all unmatched payments from database
    const payments = await db.collection('payments')
      .find({ status: { $ne: 'matched' } })
      .toArray();
    
    // Match payments with registrations
    const matchResults = await matcher.matchPayments(payments as any);
    ...
    const stats = {
      ...
      unmatched: matchResults.length - filteredResults.length,
      highConfidence: filteredResults.filter(r => r.matchConfidence >= 80).length,
      mediumConfidence: filteredResults.filter(r => r.matchConfidence >= 50 && r.matchConfidence < 80).length,
      lowConfidence: filteredResults.filter(r => r.matchConfidence < 50).length
    };
...
// Write port configuration to file for the application
...
async function startServer() {
  try {
    ...
    // Write port config for the application
