...
async function exploreCollections(): Promise<void> {
  ...
  try {
    ...
    for (const col of paymentCollections) {
      ...
      const sample: DocumentSample | null = await db.collection<DocumentSample>(col.name).findOne({});
      if (sample) {
    ...
    for (const col of collections) {
      ...
      try {
        ...
        const found: DocumentSample | null = await db.collection<DocumentSample>(col.name).findOne({
          $or: [
            { _id: searchId },
