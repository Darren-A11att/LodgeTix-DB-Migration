import { MongoClient, Db, Collection, CollStats } from 'mongodb';
...
async function setupTransactionsCollection(): Promise<void> {
  ...
  try {
    ...
    const collections: CollectionInfo[] = await db.listCollections({ name: 'transactions' }).toArray();
    ...
    const stats: CollStats = await transactionsCollection.stats();
