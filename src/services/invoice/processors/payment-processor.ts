...
export class PaymentProcessor {
  ...
  formatPaymentMethod(payment: PaymentData): string {
    ...
    let method = (payment.paymentMethod && typeof payment.paymentMethod === 'object' ? (payment.paymentMethod as any).type : payment.paymentMethod) || 
                 payment.method || 
                 'credit_card';
  ...
  private extractLast4(payment: PaymentData): string {
    // Check unified payment method structure first
    if (payment.paymentMethod && typeof payment.paymentMethod === 'object' && (payment.paymentMethod as any).last4) {
      return (payment.paymentMethod as any).last4;
    }
  ...
  private extractCardBrand(payment: PaymentData): string {
    // Check unified payment method structure first
    const brand = (payment.paymentMethod && typeof payment.paymentMethod === 'object' ? (payment.paymentMethod as any).brand : null) ||    // Unified structure
