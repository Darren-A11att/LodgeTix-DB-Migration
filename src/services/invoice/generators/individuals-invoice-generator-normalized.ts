...
export class NormalizedIndividualsInvoiceGenerator extends BaseInvoiceGenerator {
  ...
  async generateInvoice(options: InvoiceGeneratorOptions): Promise<Invoice> {
    ...
    const invoice: Invoice = {
      ...
      notes: this.generateNotes(registration)
    };
   ...
   */
  protected generateLineItems(
    options: InvoiceGeneratorOptions
  ): InvoiceItem[] {
    const { registration } = options;
    
    // Extract data directly from registration
    const attendees = registration.attendees || [];
    const tickets = registration.tickets || [];
    const confirmationNumber = registration.confirmationNumber || registration.confirmation || 'N/A';
    ...
    builder.addConfirmationHeader(confirmationNumber, functionName);
