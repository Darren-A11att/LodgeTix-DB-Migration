...
async function queryDocuments(): Promise<void> {
  ...
  try {
    ...
    console.log('\n\n3. INVOICE DOCUMENT');
    console.log('-------------------');
    const invoice: InvoiceDocument | null = await db.collection('invoices').findOne({ 
    ...
    if (invoice) {
      ...
      const transactions: TransactionDocument[] = await db.collection('transactions').find({
        $or: [
      ...
      }).toArray() as TransactionDocument[];
      ...
      if (transactions.length > 0) {
        ...
        if (invoice.invoiceNumber) {
          ...
          const txByNumber: TransactionDocument[] = await db.collection('transactions').find({
            $or: [
    ...
    if (invoice && invoice.transactionIds) {
      ...
      for (const txId of invoice.transactionIds) {
        const transaction: TransactionDocument | null = await db.collection('transactions').findOne({
          $or: [
            { _id: txId },
            { id: txId },
            { transactionId: txId }
