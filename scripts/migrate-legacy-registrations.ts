...
async function migrateLegacyRegistrations(): Promise<void> {
  ...
  try {
    ...
    const registrationsCollection: Collection = db.collection('registrations');
    ...
    const registrationsToMigrate: Registration[] = await registrationsCollection.find(query).toArray();
    ...
    for (const registration of registrationsToMigrate) {
      try {
        ...
        if (regData.tickets && Array.isArray(regData.tickets)) {
          ...
          const simpleTickets: SimpleTicket[] = [];
          
          // Group tickets by eventTicketId to aggregate quantities
          const ticketMap = new Map<string, SimpleTicket>();
          
          regData.tickets.forEach((ticket: LegacyTicket) => {
            ...
            if (ticketMap.has(eventTicketId)) {
              ...
              // Add new ticket
              let price = 0;
              if (typeof ticket.price === 'object' && ticket.price.$numberDecimal) {
        ...
        if (dryRun) {
          ...
          const result: UpdateResult = await registrationsCollection.updateOne(
        ...
        console.error(`  ✗ Error processing registration:`, (error as Error).message);
        errors.push({ 
          registration: registration.confirmationNumber || registration._id.toString(), 
    ...
    if (dryRun && registrationsToMigrate.length > 0) {
      ...
      const previewData: MigrationPreview = {
        mode: 'dry-run',
      ...
      fs.writeFileSync(sampleFile, JSON.stringify(previewData, null, 2));
