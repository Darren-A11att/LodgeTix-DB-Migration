...
async function testActiveRegistrationInvoice() {
  ...
  try {
    console.log('=== FINDING REGISTRATION WITH ACTIVE TICKETS ===\n');
    
    // Find registrations with active (sold) tickets
    const activeTickets = await db.collection('tickets').find({
      status: 'sold',
      'details.registrationId': { $exists: true }
    }).limit(5).toArray();
    ...
    // Get a registration with active tickets
    const registrationId = activeTickets[0].details.registrationId;
    ...
    console.log(`${registration.confirmationNumber} | ${registration.registrationType === 'lodge' ? 'Lodge' : 'Individuals'} for Event\n`);
