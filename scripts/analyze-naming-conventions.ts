...
interface RegistrationData {
  selected_tickets?: TicketItem[];
  selectedTickets?: TicketItem[];
  event_tickets?: TicketItem[];
  eventTickets?: TicketItem[];
  tickets?: TicketItem[];
  attendees?: any[];
}
...
interface SampleStats {
  ...
  supabaseSyncCount: number;
}

interface TicketPattern {
  hasEventTicketId: boolean;
  hasEvent_ticket_id: boolean;
  hasTicketDefinitionId: boolean;
}

async function analyzeNamingConventions(): Promise<void> {
  ...
  try {
    ...
    const coll: Collection<Registration> = db.collection('registrations');
    ...
    if (oldReg) {
      ...
      console.log('- totalAmountPaid:', (oldReg.totalAmountPaid as any)?.$numberDecimal || oldReg.totalAmountPaid);
    ...
    if (sample.length > 0) {
      const stats = sample[0] as SampleStats;
    ...
    if (ticketPatterns.length > 0) {
      ...
      ticketPatterns.forEach((doc: TicketPattern) => {
