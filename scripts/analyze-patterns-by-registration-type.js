...
async function analyzePatternsByRegistrationType() {
  try {
    ...
    const registrationTypes = await registrations.aggregate([
      {
        $group: {
    ...
    const individualPatterns = new Map();
    const lodgePatterns = new Map();
    ...
    console.error('Error analyzing patterns by registration type:', error);
  } finally {
    await client.close();
  }
}

function getStructureSignature(obj, excludePaymentFields = false) {
...
function analyzeRegistrationDataStructure(regData) {
