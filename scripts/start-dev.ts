...
async function main() {
  ...
  try {
    ...
    const webProc = startProcess('npm', ['run', 'dev:next'], 'Web', colors.yellow);
    
    // Note: Migration viewer has been removed (was in separate directory)
    ...
    setTimeout(() => {
      ...
      log(`   Web Application:   http://localhost:3005`, colors.yellow);
