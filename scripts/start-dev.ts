...
async function waitForPortConfig(maxAttempts: number = 30): Promise<PortConfig> {
...
function startProcess(command: string, args: string[], name: string, color: string): ChildProcessWithoutNullStreams {
  log(`\n📦 Starting ${name}...`, color);
...
async function main(): Promise<void> {
  ...
  try {
    ...
    log('⚠️  Could not setup migration viewer: ' + (error as Error).message, colors.yellow);
  }
  ...
  try {
    ...
    console.error(`${colors.red}❌ Failed to start:${colors.reset}`, (error as Error).message);
    apiProc.kill();
    process.exit(1);
  }
}
