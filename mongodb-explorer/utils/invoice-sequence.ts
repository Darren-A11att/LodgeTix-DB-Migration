...
export class InvoiceSequence {
     ...
     */
    async initializeCounter(sequenceName = 'invoice_number', startValue = 1000): Promise<void> {
        const counters: Collection<CounterDocument> = this.db.collection(this.counterCollection);
     ...
     */
    async getNextSequenceNumber(sequenceName = 'invoice_number'): Promise<number> {
        const counters: Collection<CounterDocument> = this.db.collection(this.counterCollection);
     ...
     */
    async getCurrentSequenceNumber(sequenceName = 'invoice_number'): Promise<number> {
        const counters: Collection<CounterDocument> = this.db.collection(this.counterCollection);
     ...
     */
    async resetSequence(sequenceName = 'invoice_number', newValue: number): Promise<void> {
        const counters: Collection<CounterDocument> = this.db.collection(this.counterCollection);
