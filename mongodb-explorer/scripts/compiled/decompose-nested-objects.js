...
class NestedObjectDecomposer {
            ...
            try {
                ...
                for (const doc of documentsWithMetadata) {
                    ...
                    if (doc.metadata &&
                        ...
                        await collection.updateOne({ _id: doc._id }, { $unset: { metadata: "" } });
                        emptyMetadataCount++;
                    }
                }
    ...
    async decomposeRegistrationData(registrationId, registrationData) {
    ...
    async decomposeOtherCollections() {
    ...
    async decomposeCollection(sourceCollection, nestedField, targetCollection, sourceType) {
    ...
    async findAndDecomposeOtherNested() {
        console.log('\nLooking for other significant nested objects...');
        const collections = await this.db.listCollections().toArray();
        for (const collectionInfo of collections) {
            if (collectionInfo.name.startsWith('decomposed_'))
                continue;
    ...
    async analyzeDocumentForNesting(collectionName, doc) {
        for (const [key, value] of Object.entries(doc)) {
            ...
            if (value && typeof value === 'object' && !Array.isArray(value)) {
                ...
                if (keyCount > 3) {
                    ...
                    const existing = this.stats.find(s => s.collectionName === collectionName &&
                        s.sourceField === key);
                    if (!existing) {
    ...
    async extractToCollection(targetCollectionName, data) {
    ...
    // Phase 4: Analysis
    async analyzeDecomposedCollections() {
