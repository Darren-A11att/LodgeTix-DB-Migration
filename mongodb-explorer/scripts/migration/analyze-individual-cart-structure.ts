...
interface AnalysisReport {
  totalCarts: number;
  cartsWithIssues: number;
  issues: {
    incompleteBundleStructure: Cart[];
    incompleteFormData: Array<{
      cart: Cart;
      bundleId: string;
      missingFields: string[];
      formData: any;
    }>;
    improperlLinkedChildren: Array<{
      cart: Cart;
      childItems: CartItem[];
    }>;
    multipleBundlesPerCart: Cart[];
  };
  examples: {
    goodFormData?: any;
    badFormData?: any;
...
async function analyzeIndividualCartStructure(): Promise<AnalysisReport> {
  ...
  try {
    ...
    for (const cart of individualCarts as any[]) {
      let hasIssues = false;

      // Get individual registration items (these act as bundles)
      const individualItems = cart.cartItems.filter(item => 
...
export { analyzeIndividualCartStructure, type AnalysisReport };