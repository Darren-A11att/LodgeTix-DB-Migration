// @ts-nocheck
/**
* Generated by MongoDB Relational Migrator
* https://www.mongodb.com/products/relational-migrator
* Collection: paymentGateway
* Language: JavaScript
* Template: Node
* Generated on 2025-06-25
*/
export const find = (database, filter) => {
    return database.collection("paymentGateway").find(filter);
}

export const findOne = (database, filter) => {
    return database.collection("paymentGateway").findOne(filter);
}

export const deleteOne = (database, filter) => {
    return database.collection("paymentGateway").deleteOne(filter);
}

export const insertOne = (database, filter) => {
    return database.collection("paymentGateway").insertOne(filter);
}

export const updateOne = (database, filter, updateDoc) => {
    return database.collection("paymentGateway").updateOne(filter, updateDoc);
}
