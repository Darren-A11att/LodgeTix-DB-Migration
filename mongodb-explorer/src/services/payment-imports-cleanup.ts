...
export class PaymentImportsCleanupService {
  private db: Db;
   ...
   */
  async getOverlapStatistics(): Promise<{
    paymentImportsTotal: number;
  ...
  async markAsProcessed(options: {
    ...
    if (!dryRun) {
      const result = await this.db.collection('payment_imports').updateMany(
        filter,
        {
          $set: {
