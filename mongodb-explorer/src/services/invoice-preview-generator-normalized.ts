...
export class NormalizedInvoicePreviewGenerator {
  ...
  async generatePreview(matchResult: MatchResult): Promise<InvoicePreview | null> {
    ...
    const processingFees = registration.registrationData?.processingFees || (registration as any).processingFees || this.calculateProcessingFees(subtotal, payment.source);
    ...
    const preview: InvoicePreview = {
      ...
      registrationDetails: {
        ...
        functionName: (registration as any).functionName || registration.registrationData?.functionName || 'Event',
  ...
  private calculateSubtotal(lineItems: InvoiceItem[]): number {
    ...
    lineItems.forEach(item => {
      ...
      if (item.subItems) {
        item.subItems.forEach((subItem: InvoiceItem) => {
          subtotal += subItem.total || 0;
        });
      }
    });
